...
يمكن تشغيل الموقع بطريقتين: من جذر المستودع حيث يحمّل `index.html` الملفات الثابتة من `./dist/`، أو برفع محتوى مجلد `dist/` وحده على أي استضافة static. يستخدم ملف `dist/index.html` مسارات نسبية (`./app.js` و`./styles.css`) حتى لا تظهر صفحة فارغة عند النشر تحت subpath.
